...
const Auth = {
    ...
    handleSocialAuth(provider) {
        ...
        const finalizeSocialLogin = async () => {
            ...
            try {
                const response = await fetch('auth_handler.php', { method: 'POST', body: formData });
    ...
    async submitOTP(contact, action) {
        ...
        try {
            const res = await fetch('auth_handler.php', { method: 'POST', body: formData });
    ...
    async checkLoginStatus() {
        try {
            const response = await fetch('auth_handler.php?check=1');
            const data = await response.json();
            if (data.loggedIn) {
                this.updateUserUI(true, data.user.first_name, data.user.is_admin == 1, data.user);
            }
